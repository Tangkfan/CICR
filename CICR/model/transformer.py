...
def gen_sineembed_for_position(pos_tensor, dim):
    ...
    scale = 2 * math.pi
    each_dim = dim // 2
    dim_t = torch.arange(each_dim, dtype=torch.float32, device=pos_tensor.device)
    dim_t = 10000 ** (2 * torch.div(dim_t, 2, rounding_mode='trunc') / each_dim)
...
class T2VEncoder(nn.Module):
    ...
    def forward(self,
        ...
        return src_vid_output.permute(1, 0, 2)
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, global_token, global_token_pos):
        ...
        """
        global_token_mask = torch.tensor([True], device=mask.device).repeat(mask.shape[0], 1)
        mask = torch.cat([global_token_mask, mask], dim=1).bool()
        src = torch.cat([global_token, src], dim=1)
        pos_embed = torch.cat([global_token_pos, pos_embed], dim=1)
...
class T2V_TransformerEncoder(nn.Module):
    ...
    def forward(self,
                ...
                pos_vid: Optional[Tensor] = None,
                **kwargs):
        output = src_vid  # L, b, D
        intermediate = []
        for layer in self.layers:
            output = layer(src_txt, output,
                           src_txt_mask, src_txt_key_padding_mask, pos_txt,
                           src_vid_mask, src_vid_key_padding_mask, pos_vid,
                           **kwargs)
    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            output = layer(output, src_mask=mask,
                           src_key_padding_mask=src_key_padding_mask, pos=pos, stage1=stage1, **kwargs)
            if self.return_intermediate:
                intermediate.append(output)
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        pos_src_txt = self.with_pos_embed(src_txt, pos_txt)  # n, b, d
        pos_src_vid = self.with_pos_embed(src_vid, pos_vid)  # l, b, d
        q = pos_src_vid
        k = pos_src_txt
        v = src_txt
        qmask = src_vid_key_padding_mask.unsqueeze(2)  # b, l, 1
        kmask = src_txt_key_padding_mask.unsqueeze(1)  # b, 1, n
        attn_mask = torch.matmul(qmask.float(), kmask.float()).bool().repeat(self.nhead, 1, 1)  # b * nhead, l, n

        src2 = self.self_attn(q, k, value=v, attn_mask=attn_mask,
                              key_padding_mask=src_txt_key_padding_mask)[0]  # l, b, d
        src2 = src_vid + self.dropout1(src2)  # l, b, d

        src3 = self.norm1(src2)  # l, b, d
        src3 = self.linear2(self.dropout(self.activation(self.linear1(src3))))  # Video MLP   l, b, d
        src2 = src2 + self.dropout2(src3)
        src2 = self.norm2(src2)  # enhanced frame-level features
        return src2
    ...
    def forward(self,
                ...
                **kwargs):
        if self.normalize_before:
            raise NotImplementedError("self.normalize_before is True")
            # return self.forward_pre(src, src_mask, src_key_padding_mask, pos)
        # For tvsum, add kwargs
        return self.forward_post(src_txt, src_vid,
                                 src_txt_mask, src_txt_key_padding_mask, pos_txt,
                                 src_vid_mask, src_vid_key_padding_mask, pos_vid,
                                 **kwargs)


class T2V_TransformerEncoderLayer_TwoMLP(T2V_TransformerEncoderLayer):
    ...
    def forward_post(self,
        ...
        pos_src_vid = self.with_pos_embed(src_vid, pos_vid)
        q = pos_src_vid
        k = pos_src_txt
        v = src_txt
        qmask = src_vid_key_padding_mask.unsqueeze(2)
        kmask = src_txt_key_padding_mask.unsqueeze(1)
        attn_mask = torch.matmul(qmask.float(), kmask.float()).bool().repeat(self.nhead, 1, 1)

        src2 = self.self_attn(q, k, value=v, attn_mask=attn_mask,
                              key_padding_mask=src_txt_key_padding_mask)[0]
        src2 = src_vid + self.dropout1(src2)
        ...
        if not is_MLM:
            ...
            src3 = self.norm1_1(src2)
            src3 = self.linear2_1(self.dropout_1(self.activation(self.linear1_1(src3))))
            src2 = src2 + self.dropout2_1(src3)
            src2 = self.norm2_1(src2)
