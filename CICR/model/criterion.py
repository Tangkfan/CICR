...
class Criterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses,
                 ...
                 saliency_margin=1,
                 multi_clip=False,
                 gamma=0.9, recss_tau=0.5, is_vy=False):
    ...
    def loss_spans(self, outputs, targets, indices):
        ...
        if self.multi_clip:
            ...
            if self.is_vy:
                # src_spans_all = outputs['pred_spans_all'][idx]
                src_spans_v = outputs['pred_spans_cf'][idx]
        else:
            ...
            if self.is_vy:
                # src_spans_all = outputs['pred_spans_all'][batch_idx, indices[:, 0], :]  # (#spans, max_v_l * 2)
                src_spans_cf = outputs['pred_spans_cf'][batch_idx, indices[:, 0], :]  # (#spans, max_v_l * 2)
        if self.span_loss_type == "l1":
            loss_span = F.l1_loss(src_spans, tgt_spans, reduction='none')
            loss_giou = 1 - torch.diag(generalized_temporal_iou(span_cxw_to_xx(src_spans), tgt_moments))
            if self.is_vy:
                # loss_span_all = F.l1_loss(src_spans_all, tgt_spans, reduction='none')
                loss_span_cf = F.l1_loss(src_spans_cf, tgt_spans, reduction='none')
                # loss_giou_all = 1 - torch.diag(generalized_temporal_iou(span_cxw_to_xx(src_spans_all), tgt_moments))
                loss_giou_cf = 1 - torch.diag(generalized_temporal_iou(span_cxw_to_xx(src_spans_cf), tgt_moments))
        else:  # ce
            ...
            if self.is_vy:
                src_spans_cf = src_spans_cf.view(n_spans, 2, self.max_video_l).transpose(1, 2)
                loss_span_cf = F.cross_entropy(src_spans_cf, tgt_spans, reduction='none')
                # src_spans_v = src_spans_v.view(n_spans, 2, self.max_video_l).transpose(1, 2)
                # loss_span_all = F.cross_entropy(src_spans_v, tgt_spans, reduction='none')
        ...
        losses = {}
        if self.is_vy:
            losses['loss_span'] = loss_span.mean() + loss_span_cf.mean()
            losses['loss_giou'] = loss_giou.mean() + loss_giou_cf.mean()
        else:
