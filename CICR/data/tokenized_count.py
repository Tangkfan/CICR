...
def count_anetcd_tokenized_id(ann_path, split, max_words_l, tokenizer=None):
    def _load_durations():
        split2filename = {
            "train": "anet_train_momentdiff.jsonl",
            "val": "anet_test_iid_momentdiff.jsonl",
            "test": "anet_test_ood_momentdiff.jsonl",
        }
        ann_file = os.path.join(ann_path, split2filename[split])
        with open(ann_file, "r") as f:
            datalist = [json.loads(l.strip("\n")) for l in f.readlines()]
        with open(ann_file, 'r') as f:
            csv_reader = csv.reader(f, delimiter=',')
            first_line_flag = True
            durations = dict()
...
def count_GloVe_tokenizer():
    ...
    #     tokenizer=None,
    # )
    vocab_dict = count_anetcd_tokenized_id(
        ann_path="/media/hdd2/tkf/CICR/CIDETR/CIDETR_QV/data/anet_cd/annotations",
        split="train",
        max_words_l=16,
        tokenizer=None,
    )
    val_vocab_dict = count_anetcd_tokenized_id(
        ann_path="/media/hdd2/tkf/CICR/CIDETR/CIDETR_QV/data/anet_cd/annotations",
        split="test",
        max_words_l=16,
        tokenizer=None,
