...
def process_query_oie_clip(args):
    ...
    with open(args.annotation_file, 'r') as f:
        lines = f.readlines()
    instances = []
    for i in tqdm(range(len(lines)), desc=f"Load Charades {args.mode} annotations"):
        meta = lines[i].split("##")
        ...
        if args.mode in ['train']:
            ...
            for instance in tqdm(instances):
                ...
                for token in nltk.word_tokenize(token_subject):
                    if token not in query_oie_subject_token_to_id:
                        query_oie_subject_token_to_id[token] = len(query_oie_subject_token_to_id)

                # relation
                for token in nltk.word_tokenize(token_relation):
                    if token not in query_oie_relation_token_to_id:
                        query_oie_relation_token_to_id[token] = len(query_oie_relation_token_to_id)

                # object
                for token in nltk.word_tokenize(token_object):
                    if token not in query_oie_object_token_to_id:
                        query_oie_object_token_to_id[token] = len(query_oie_object_token_to_id)
