...
class CLIPTokenizer(object):
    def __init__(self, id2label, bpe_path: str = default_bpe()):
...
class GloVeSimpleTokenizer():
    def __init__(self, id2label, vocab):
        self.id2label = id2label
        self.vocab = vocab
    ...
    def tokenize(self, texts: Union[str, List[str]], max_valid_length: int = 32):
        if isinstance(texts, str):
            texts = [texts]

        all_tokens = []
        for text in texts:
            txt_token = self.encode(text)
...
class NLTKTokenizerWithFeature():
    ...
    def tokenize(self, texts: Union[str, List[str]]):
        ...
        all_tokens = []
        for text in texts:
            words, words_feat = self.encode(text)
            if not words:
                continue
        ...
        result = torch.zeros(len(all_tokens), words_feat.shape[-1])
