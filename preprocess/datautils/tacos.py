...
def process_query_oie(args, vocab):
    ...
    with StanfordOpenIE(properties=properties) as client:
        if args.mode in ['train']:
            ...
            for instance in tqdm(instances):
                ...
                # subject
                sub_words_id = \
                    args.tokenizer.tokenize(token_subject, max_valid_length=args.max_words_l)
                ...
                # relation
                rel_words_id = \
                    args.tokenizer.tokenize(token_relation, max_valid_length=args.max_words_l)
                ...
                # object
                obj_words_id = \
                    args.tokenizer.tokenize(token_object, max_valid_length=args.max_words_l)
        ...
        sub_words_id = torch.tensor(sub_words_id, dtype=torch.int).unsqueeze(0)
        sub_words_feat, sub_words_id = GloVe_encode_text(args, sub_words_id, vocab)
        obj = {
            'feat': sub_words_feat,
        ...
        rel_words_id = torch.tensor(rel_words_id, dtype=torch.int).unsqueeze(0)
        rel_words_feat, rel_words_id = GloVe_encode_text(args, rel_words_id, vocab)
        obj = {
            'feat': rel_words_feat,
        ...
        obj_words_id = torch.tensor(obj_words_id, dtype=torch.int).unsqueeze(0)
        obj_words_feat, obj_words_id = GloVe_encode_text(args, obj_words_id, vocab)
        obj = {
            'feat': obj_words_feat,
